...
const WishList = () => {
    ...
    const getPrdctDetails = async () => {
        const res = await axios.get(`http://localhost:2000/footwear/getWishlistProduct/${id}`);
        setProdct(res.data);
    ...
    const delProduct = async (id) => {
        ...
        if (userConfirmed) {
            try {
                const res = await axios.delete(`http://localhost:2000/footwear/delWishListProduct/${id}`);
